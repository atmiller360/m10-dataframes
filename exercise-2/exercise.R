...
sal1 <- runif(length(employees), 20000, 80000)

# Create a vector of 2015 salaries that are typically higher than the 2014 salaires (use runif again)
sal2 <- runif(length(employees), 40000, 100000)

# Create a data.frame 'salaries' by combining the vectors you just made
salaries <- data.frame(sal1, sal2)

# Create a column 'raise' that stores the size of the raise between 2014 and 2015
salaries$raise <- sal2 - sal1

# Create a column 'got.raise' that is TRUE if the person got a raise
salaries$got.raise <- sal2 > sal1
...
len <- length(salaries[salaries$got.raise, 'sal1'])
